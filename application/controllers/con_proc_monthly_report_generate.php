...
// Add some data
        ...
        for ($index = 0; $index <= $limit; $index++) {
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('T' . ($index + 2), $tbl_monthly_wages_detail[$index]['OverTimeHour'] 
                                                     + $tbl_monthly_wages_detail[$index]['AdditionalOverTimeHour'] 
                                                     + $tbl_monthly_wages_detail[$index]['NightShiftOverTimeHour'])
                    ->setCellValue('U' . ($index + 2), $tbl_monthly_wages_detail[$index]['HourlyOTWage'])
                    ->setCellValue('V' . ($index + 2), ($tbl_monthly_wages_detail[$index]['OverTimeHour'] + $tbl_monthly_wages_detail[$index]['AdditionalOverTimeHour'] + $tbl_monthly_wages_detail[$index]['NightShiftOverTimeHour']) 
                                                       * $tbl_monthly_wages_detail[$index]['HourlyOTWage'])
...
//            echo 'Holiday Work Payable : ' . $tbl_monthly_wages_detail[$index]['HolidayNetPayable'] . '<br/>';
            ...
            $tbl_monthly_wages_detail[$index]['TotalAvailableToPay'] = number_format((float) ((($tbl_monthly_wages_detail[$index]['Basic'] / $month_days) * ($month_days - $tbl_monthly_wages_detail[$index]['Absent'])) 
                                                                                              + $tbl_monthly_wages_detail[$index]['HouseRent'] 
                                                                                              + $tbl_monthly_wages_detail[$index]['MedicalAllowance'] 
                                                                                              + $tbl_monthly_wages_detail[$index]['TravelAllowance'] 
                                                                                              + $tbl_monthly_wages_detail[$index]['FoodAllowance']), 2, '.', '');
            $total = number_format((float) ($tbl_monthly_wages_detail[$index]['HolidayNetPayable'] 
                                          + $tbl_monthly_wages_detail[$index]['TotalAvailableToPay'] 
                                          + $tbl_monthly_wages_detail[$index]['OutStandingDues'] 
                                          + $total_ot_salary 
                                          + $tbl_monthly_wages_detail[$index]['OtherAllowance'] 
                                          + $tbl_monthly_wages_detail[$index]['AttendanceBonus'] 
                                          + ($no_of_tiffin * $tiffin_allowance)), 2, '.', '');
